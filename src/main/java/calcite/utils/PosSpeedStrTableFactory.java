...
public class PosSpeedStrTableFactory implements TableFactory<Table>  {
	/*
	 ...
	 */
		...
		public Table create(SchemaPlus schema, String name, Map<String, Object> operand, RelDataType rowType) {
			final Object[][] rows = {
				...
				return new PosSpeedStrTable(ImmutableList.copyOf(rows), useRatesCostModel);
		}
		...
		public static class PosSpeedStrTable implements ScannableTable {
			protected final RelProtoDataType protoRowType = new RelProtoDataType() {
				public RelDataType apply(RelDataTypeFactory a0) {
			        return a0.builder()
			        	.add("rowtime", SqlTypeName.TIMESTAMP)
			            .add("vehicle", SqlTypeName.INTEGER)
			            .add("speed", SqlTypeName.FLOAT)
			            .add("highway", SqlTypeName.INTEGER)
			            .add("lane", SqlTypeName.INTEGER)
			            .add("direction", SqlTypeName.INTEGER)
			            .add("pos", SqlTypeName.INTEGER)
			            .build();
				}
			};
			...
			public PosSpeedStrTable(ImmutableList<Object[]> rows, boolean useRatesCostModel) {
				this.rows = rows;
				this.useRatesCostModel = useRatesCostModel;
			}
