...
public class VolcanoPhysicalRuleConverter {
	
	private final static Logger log = LogManager.getLogger (VolcanoPhysicalRuleConverter.class);
	...
	public VolcanoPhysicalRuleConverter (RelNode logicalPlan, Map<String, Pair<ITupleSchema,Pair<byte [],ByteBuffer>>> tablesMap , SystemConf systemConf, long timestampReference) {
	...
	public Pair<Integer, String> convert (RelNode logicalPlan) {
		 ...
		 * 1)If children.size() == 0, we have a leaf node that is a simple EnumerableTableScan.
		...
		if (children.size() == 0){	
			...
			VolcanoRuleAssembler operation = new VolcanoRuleAssembler(chainTail.getRelTypeName(), null, pair.left, chainTail.getId(), timestampReference);	    
		...
		if (children.size() == 1) {
			...
			if (logicalPlan.getRelTypeName().equals("EnumerableFilter")) {
				args = logicalPlan.getChildExps().toString();
				//System.out.println(args);
			} else
			if (logicalPlan.getRelTypeName().equals("EnumerableAggregate")) {
				args = logicalPlan.getDescription().replace("input="+logicalPlan.getInput(0).toString()+",", "");
				args = args.substring(args.indexOf("("));
				//System.out.println(args);				
			} else
			if (logicalPlan.getRelTypeName().equals("EnumerableWindow")) {
				System.err.println("EnumerableWindow is not supported yet.");
				System.exit(1);
			} else {
			...
			VolcanoRuleAssembler operation = new VolcanoRuleAssembler(logicalPlan.getRelTypeName(), args, chain.getOutputSchema(), queryId, timestampReference);
			SaberRule rule = operation.construct();
			...
			if (!(node.right.equals("EnumerableTableScan"))) {
		...
		if (children.size() == 2) {
			...
			VolcanoRuleAssembler operation = new VolcanoRuleAssembler(logicalPlan.getRelTypeName(), args, leftChain.getOutputSchema(), rightChain.getOutputSchema(), queryId, timestampReference);
			SaberRule rule = operation.construct();
			Query query = rule.getQuery();
			System.out.println("Current query id : "+ query.getId());
			queryId++; //increment the queryId for the next query
			if ((!(leftNode.right.equals("EnumerableTableScan"))) && (!(rightNode.right.equals("EnumerableTableScan")))) {
